"""Simulate ten satellites with four-channel orbital telemetry."""
...
def simulate_dataset(samples_per_satellite: int = 10_000, random_seed: int = 42) -> pd.DataFrame:
    ...
    # 10 Hz sampling and 90-minute LEO orbital cycle.
    ...
    for sat_ix, sat in enumerate(satellites):
        ...
        orbital = np.sin(2 * np.pi * t / orbital_period_samples + phase)
        power = 120 + gain * 9.0 * orbital + rng.normal(0, 1.2, samples_per_satellite)
        temp = 24 + gain * 2.8 * np.sin(2 * np.pi * t / orbital_period_samples + phase / 2) + rng.normal(0, 0.5, samples_per_satellite)
        voltage = 28 + gain * 0.7 * np.cos(2 * np.pi * t / orbital_period_samples) + rng.normal(0, 0.12, samples_per_satellite)
        wheel = 3600 + gain * 180 * np.sin(2 * np.pi * t / 500 + phase) + rng.normal(0, 25, samples_per_satellite)
